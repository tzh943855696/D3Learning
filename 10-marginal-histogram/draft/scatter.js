async function drawScatter() {
  ...
  const topHistogramGenerator = d3.histogram().domain(xScale.domain())
  .value(xAccessor)
  .thresholds(50)

  const topHistogramBins = topHistogramGenerator(dataset)
  console.log(topHistogramBins)
  ...
  const topHistogramBounds = bounds.append("g")
    ...
    const topHistogramLineGenerator = d3.area().x(d => xScale((d.x0 + d.x1) / 2)).y0(dimensions.histogramHeight)
    .y1(d => topHistogramYScale(d.length)) .curve(d3.curveBasis)
  
  const rightHistogramGenerator = d3.histogram().domain(yScale.domain())
  .value(yAccessor)
  .thresholds(20)
  ...
  .range([dimensions.histogramHeight, 0])
  const rightHistogramBounds = bounds.append("g") .attr("class", "right-histogram") .style("transform", `translate(${
    dimensions.boundedWidth + dimensions.histogramMargin }px, -${
    dimensions.histogramHeight }px) rotate(90deg)`)


    const rightHistogramLineGenerator = d3.area() .x(d => yScale((d.x0 + d.x1) / 2)) .y0(dimensions.histogramHeight)
    .y1(d => rightHistogramYScale(d.length)) .curve(d3.curveBasis)
  // 5. Draw data
